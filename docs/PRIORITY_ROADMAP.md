...
| **11c** | **Verify-chunk numerical alignment (K=3 ↔ K=1)** — fp16 argmaxes from `verify_qK` and `predict` diverge under speculation by design (different SIMD tiling). Investigate whether reconverting verify chunks to match the decode chunk's accumulation order, or normalising via an explicit fp32 logit cast, closes the gap. ~~**Upgraded 2026-04-15** — Path C + Union confluence made this load-bearing for any speculative drafter.~~ **Downgraded 2026-04-15 late by PR #62.** Mac reproduction (`coreml-llm-smoke UNION_TRIP=1`) shows the same live-vs-oracle accept-rate gap on Mac CPU+GPU where ANE fp16 drift does not apply — so the drift was not the primary driver of Path C / Union regressions. 11c is still real (PR #54 thread's bit-exact criterion is valid), but its fix would not lift live accept rates meaningfully. Keep tracked for correctness reasons, not speed. | correctness interesting; **not** the speculation-speed unlock | 1–2 days investigation + possible reconvert | PR #54 thread, `docs/MTP_PATH_C_FINDINGS.md`, `docs/PHASE_B_LIVE_ACCEPT_RATE_GAP.md` |
