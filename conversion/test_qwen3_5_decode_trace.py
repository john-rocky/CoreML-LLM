...
class DecodeStepModule(nn.Module):
    ...
    Inputs:
        ...
        conv_state:    (1, C, K-1)         - previous conv1d state (tail window)
        recurrent_state: (1, Hv, Dk, Dv)   - SSM recurrent state

    Outputs:
        hidden_out:     (1, 1, H)           - layer output (pre-residual / norm)
        new_conv_state: (1, C, K-1)
        new_rec_state:  (1, Hv, Dk, Dv)

    Math mirrors Qwen3NextGatedDeltaNet.forward in use_precomputed_states=True mode
    ...
    def forward(self, hidden_in, conv_state, rec_state):
        ...
        # 2. causal_conv1d_update: concat(conv_state, current) along last dim,
        #    depthwise-mul by conv1d weights (no bias), SiLU, output last position.
        mixed_qkv_t = mixed_qkv.transpose(1, 2)               # (1,C,1)
        w = torch.cat([conv_state, mixed_qkv_t], dim=-1)      # (1,C,K)
        kw = self.conv_w.squeeze(1)                            # (C,K)
        conv_out = (w * kw.unsqueeze(0)).sum(dim=-1, keepdim=True)  # (1,C,1)
        conv_out = F.silu(conv_out)
        new_conv_state = w[:, :, 1:]                          # (1,C,K-1)
...
def extract_hf_decode_io(hf_model, prompt_ids, tok_idx: int):
    ...
    def pre_hook(module, args, kwargs):
        captured["input_hidden"] = args[0].detach().clone()

    def post_hook(module, args, kwargs, output):
        captured["output_hidden"] = output.detach().clone()
    ...
    if layer_cache is None:
        # No prior state: use zeros
        conv_state = torch.zeros(1, lin.conv_dim, lin.conv_kernel_size - 1)
...
def main():
    ...
    example_in = (
        ...
        torch.zeros(1, decode_mod.conv_dim, decode_mod.K - 1),
