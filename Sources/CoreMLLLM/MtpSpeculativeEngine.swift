...
public final class MtpSpeculativeEngine {
    ...
    func speculateStep(nextID: inout Int32) throws -> [Int32] {
        ...
        if adaptive {
            ...
            // slot for argmax comparison). Verify produces K argmaxes:
            // slots 0..K-2 verify drafted tokens, slot K-1 is the bonus.
            // For K=3 this gives 2 (matches old hard-coded default that
            // empirically beat adaptive). For K=6 it gives 5, exploiting
            // the wider verify cap on prompts where drafter accept is high.
